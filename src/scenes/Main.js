...
class Main extends Phaser.Scene {
    ...
    for (let i = 0; i < rows; i++) {
      for (let j = 0; j < columns; j++) {
        let brick = this.physics.add.image(
          53 + (xOffset * (j + 1) + width * j),
          40 + (yOffset * (i + 1) + height * i),
          "brick"
        );
  ...
  placeBase() {
    ...
    base.setBounce(1).setCollideWorldBounds(true);
  ...
  placeBall() {
    ...
    ball
      .setVelocity(0, 0) //initial velocity
  ...
  movementManager() {
    ...
    if (
      this.keyControls.left.isDown === true &&
      this.keyControls.right.isDown === true
    ) {
      this.stopBase();
      return;
    }
  ...
  retry() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire("Thank you for Playing!");
      }
    });
  }
  resetBall() {
    this.ball.setVelocity(0, 0);
    this.ball.setPosition(400 - 50, 450);
  }
